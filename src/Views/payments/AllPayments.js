...
const StatusBadge = ({ status, dm }) => {
  ...
  const s = cfg[mappedStatus] || cfg.pending;
  return (
...
const AllPayments = ({ darkMode }) => {
  ...
  const exportCSV = () => {
    ...
    const rows   = getFiltered().map((p) => [
      safeStr(p?.bookingId), safeStr(p?.farmhouse?.name), safeStr(p?.user?.name, 'Unknown'),
  ...
  const getFiltered = () => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((p) =>
        safeStr(p?.farmhouse?.name).toLowerCase().includes(s) ||
        safeStr(p?.user?.name).toLowerCase().includes(s)      ||
        safeStr(p?.bookingId).toLowerCase().includes(s)
      );
    }
    list.sort((a, b) => {
      let av, bv;
      ...
      else if (sortField === 'farmhouse') { av = safeStr(a?.farmhouse?.name); bv = safeStr(b?.farmhouse?.name); }
      else { av = 0; bv = 0; }
  ...
  const statusObj = overview?.status || {};
  const charts = data?.charts || {};
  const daily = safeArr(charts?.daily);
  const paymentMethods = safeArr(data?.paymentMethods);
  const refunds = data?.refunds || {};

  const totalPayments = safeNum(overview?.totalPayments);
  const totalRevenue = safeNum(overview?.totalRevenue);
  const averagePayment = safeNum(overview?.averagePayment);
  const successRate = totalPayments > 0 ? Math.round((safeNum(statusObj?.completed) / totalPayments) * 100) : 0;
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${bg}`}>
      ...
      <div className="p-6 md:p-8 space-y-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            { label: 'Total Payments', value: totalPayments, sub: `${successRate}% success rate`, grad: 'from-lime-500 to-lime-600', icon: CreditCard },
            { label: 'Total Revenue',  value: fmtMon(totalRevenue), sub: `Avg ${fmtMon(averagePayment)}/payment`, grad: 'from-amber-500 to-amber-600', icon: DollarSign },
            { label: 'Completed',      value: safeNum(statusObj?.completed), sub: `Pending: ${safeNum(statusObj?.pending)} · Failed: ${safeNum(statusObj?.failed)}`, grad: 'from-lime-600 to-lime-700', icon: CheckCircle },
              ...
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={daily} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: dm ? '#1c1917' : '#fff', borderColor: dm ? '#44403c' : '#d9f99d', borderRadius: 12, fontSize: 12 }}
                    formatter={(v, n) => [n === 'revenue' ? fmtMon(v) : v, n === 'revenue' ? 'Revenue' : 'Payments']} />
          ...
          <SectionCard dm={dm}>
            ...
            {paymentMethods.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie
                    data={paymentMethods.map((m) => ({ name: safeStr(m?.method, 'Unknown'), value: safeNum(m?.amount) }))}
                    cx="50%" cy="50%" innerRadius={55} outerRadius={90} paddingAngle={4} dataKey="value"
                  >
                    {paymentMethods.map((_, i) => <Cell key={i} fill={LIME_COLORS[i % LIME_COLORS.length]} />)}
                  </Pie>
                  <Tooltip contentStyle={{ backgroundColor: dm ? '#1c1917' : '#fff', borderColor: dm ? '#44403c' : '#d9f99d', borderRadius: 12, fontSize: 12 }}
          ...
          <div className={`p-5 border-b ${dm ? 'border-stone-700' : 'border-lime-100'}`}>
            <div className="flex flex-wrap gap-3 items-center justify-between">
              <div className="relative flex-1 min-w-[200px] max-w-sm">
                ...
                <input
                  placeholder="Search farmhouse, customer, ID…"
                  value={search}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className={`border-b ${dm ? 'bg-stone-700/40 border-stone-700' : 'bg-lime-50 border-lime-100'}`}>
                  <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest text-lime-600 dark:text-lime-400/70">Payment ID</th>
                  <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest cursor-pointer select-none text-lime-600 dark:text-lime-400/70 hover:text-lime-800 dark:hover:text-lime-400">
                    <span className="flex items-center gap-1.5" onClick={() => toggleSort('farmhouse')}>
                      Farmhouse
                      <ArrowUpDown className={`h-3 w-3 ${sortField === 'farmhouse' ? 'opacity-100' : 'opacity-30'}`} />
                    </span>
                  </th>
                  <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest text-lime-600 dark:text-lime-400/70">Customer</th>
                  <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest cursor-pointer select-none text-lime-600 dark:text-lime-400/70 hover:text-lime-800 dark:hover:text-lime-400">
                    <span className="flex items-center gap-1.5" onClick={() => toggleSort('date')}>
                      Date
                      <ArrowUpDown className={`h-3 w-3 ${sortField === 'date' ? 'opacity-100' : 'opacity-30'}`} />
                    </span>
                  </th>
                  <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest cursor-pointer select-none text-lime-600 dark:text-lime-400/70 hover:text-lime-800 dark:hover:text-lime-400">
                    <span className="flex items-center gap-1.5" onClick={() => toggleSort('amount')}>
                      Amount
                      <ArrowUpDown className={`h-3 w-3 ${sortField === 'amount' ? 'opacity-100' : 'opacity-30'}`} />
                    </span>
                  </th>
                  <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest text-lime-600 dark:text-lime-400/70">Status</th>
                  <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest text-lime-600 dark:text-lime-400/70">View</th>
                </tr>
              </thead>
              <tbody>
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-16 opacity-40">
                      ...
                      <p className={`text-sm ${dm ? 'text-stone-400' : 'text-stone-500'}`}>No payments found</p>
                    </td>
                  </tr>
                ) : (
                  paginated.map((p) => {
                    const id = safeStr(p?.bookingId);
                    const farm = safe(p?.farmhouse?.name);
                    const customer = safe(p?.user?.name, 'Unknown');
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-4">
                          <code className={`text-xs font-mono px-2 py-1 rounded-lg ${dm ? 'bg-lime-500/10 text-lime-300' : 'bg-lime-50 text-lime-700'}`}>
                            {id ? `…${id.slice(-8)}` : '—'}
                          </code>
                        </td>
                        <td className="px-5 py-4">
                        ...
                        <td className="px-5 py-4">
                          <button
                            ...
                            className={`p-2 rounded-xl border transition hover:scale-110 ${dm ? 'border-lime-500/20 text-lime-400 hover:bg-lime-500/10' : 'border-lime-300 text-lime-600 hover:bg-lime-50'}`}
                          >
